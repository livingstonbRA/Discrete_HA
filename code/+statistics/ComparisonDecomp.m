		...
		function initialize(obj)
			...
			for ia = 1:obj.nthresholds
				...
				obj.results.term2a(ia) = nfill(...
					sprintf('Effect of distr, HtM (a <= %g)', thresh), 3,...
					'\quad Distr effect, HtM\textsuperscript{$\\dagger$}');
				obj.results.term2b(ia) = nfill(...
					sprintf('Effect of distr, NHtM (a > %g)', thresh), 3,...
					'\quad Distr effect, NHtM\textsuperscript{$\\dagger$}');

				obj.results.term2a_pct(ia) = nfill(...
					sprintf('Effect of distr (%%), HtM (a <= %g)', thresh), 1,...
					'\quad Distr effect, HtM\textsuperscript{$\\dagger$}');
				obj.results.term2b_pct(ia) = nfill(...
					sprintf('Effect of distr (%%), NHtM (a > %g)', thresh), 1,...
					'\quad Distr effect, NHtM\textsuperscript{$\\dagger$}');
			end
		end
